...
static struct {
	...
	{ CAMEL_VTRASH_NAME, N_("Trash"), "#.trash", CAMEL_MESSAGE_DELETED, CAMEL_FOLDER_IS_TRASH,
	  N_("Cannot copy messages to the Trash folder") },
	{ CAMEL_VJUNK_NAME, N_("Junk"), "#.junk", CAMEL_MESSAGE_JUNK, CAMEL_FOLDER_IS_JUNK,
...
camel_vtrash_folder_new (CamelStore *parent_store, camel_vtrash_folder_t type)
{
	...
	camel_vee_folder_construct(CAMEL_VEE_FOLDER (vtrash), parent_store, vdata[type].vid, vdata[type].full_name, _(vdata[type].name),
