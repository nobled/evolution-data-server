...
/* camel-groupwise-message-cache.c: Class for an message cache */
...
static void finalize (CamelGroupwiseMessageCache *cache);
static void stream_finalize (CamelObject *stream, gpointer event_data, gpointer user_data);


CamelType
camel_groupwise_message_cache_get_type (void)
{
	static CamelType camel_groupwise_message_cache_type = CAMEL_INVALID_TYPE;
	
	if (camel_groupwise_message_cache_type == CAMEL_INVALID_TYPE) {
		camel_groupwise_message_cache_type = camel_type_register (
			CAMEL_OBJECT_TYPE, "CamelGroupwiseMessageCache",
			sizeof (CamelGroupwiseMessageCache),
			sizeof (CamelGroupwiseMessageCacheClass),
			NULL,
			NULL,
			NULL,
	...
	return camel_groupwise_message_cache_type;
}
...
static void
finalize (CamelGroupwiseMessageCache *cache)
{
	if (cache->path)
		g_free (cache->path);
	if (cache->parts) {
...
static void
cache_put (CamelGroupwiseMessageCache *cache, const char *uid, const char *key,
	   CamelStream *stream)
{
	char *hash_key;
	GPtrArray *subparts;
			      ...
			      CamelException *ex)
{
	...
	cache = (CamelGroupwiseMessageCache *)camel_object_new (CAMEL_GW_MESSAGE_CACHE_TYPE);
...
/**
 * camel_groupwise_message_cache_max_uid:
 * @cache: the cache
 *
 * Return value: the largest (real) UID in the cache.
 **/
guint32
camel_groupwise_message_cache_max_uid (CamelGroupwiseMessageCache *cache)
{
	return cache->max_uid;
}

/**
 * camel_groupwise_message_cache_set_path:
 * @cache: 
 * @path: 
 * 
 ...
 **/
void
camel_groupwise_message_cache_set_path (CamelGroupwiseMessageCache *cache, const char *path)
{
	g_free(cache->path);
	cache->path = g_strdup(path);
}
...
{
	CamelGroupwiseMessageCache *cache = user_data;
	char *key;
...
static CamelStream *
insert_setup (CamelGroupwiseMessageCache *cache, const char *uid, const char *part_spec,
...
static CamelStream *
insert_finish (CamelGroupwiseMessageCache *cache, const char *uid, char *path,
...
/**
 * camel_groupwise_message_cache_insert:
 * @cache: the cache
 * @uid: UID of the message data to cache
 * @part_spec: the GW part_spec of the data
 * @data: the data
 * @len: length of @data
 *
 ...
 **/
CamelStream *
camel_groupwise_message_cache_insert (CamelGroupwiseMessageCache *cache, const char *uid,
...
/**
 * camel_groupwise_message_cache_insert_stream:
 * @cache: the cache
 * @uid: UID of the message data to cache
 * @part_spec: the GW part_spec of the data
 ...
 **/
void
camel_groupwise_message_cache_insert_stream (CamelGroupwiseMessageCache *cache,
...
/**
 * camel_groupwise_message_cache_insert_wrapper:
 * @cache: the cache
 * @uid: UID of the message data to cache
 * @part_spec: the GW part_spec of the data
 ...
 **/
void
camel_groupwise_message_cache_insert_wrapper (CamelGroupwiseMessageCache *cache,
					 const char *uid, const char *part_spec,
...
/**
 * camel_groupwise_message_cache_get:
 * @cache: the cache
 ...
 **/
CamelStream *
camel_groupwise_message_cache_get (CamelGroupwiseMessageCache *cache, const char *uid,
...
/**
 * camel_groupwise_message_cache_remove:
 * @cache: the cache
 ...
 **/
void
camel_groupwise_message_cache_remove (CamelGroupwiseMessageCache *cache, const char *uid)
{
	GPtrArray *subparts;
	char *key, *path;
	CamelObject *stream;
...
/**
 * camel_groupwise_message_cache_clear:
 * @cache: the cache
 *
 * Removes all cached data from @cache.
 **/
void
camel_groupwise_message_cache_clear (CamelGroupwiseMessageCache *cache)
{
	GPtrArray *uids;
	int i;
	...
	for (i = 0; i < uids->len; i++)
		camel_groupwise_message_cache_remove (cache, uids->pdata[i]);
	g_ptr_array_free (uids, TRUE);
}


/**
 * camel_groupwise_message_cache_copy:
 ...
 **/
void
camel_groupwise_message_cache_copy (CamelGroupwiseMessageCache *source,
			       ...
			       CamelException *ex)
{
	...
	for (i = 0; i < subparts->len; i++) {
		...
		if ((stream = camel_groupwise_message_cache_get (source, source_uid, part, ex))) {
			camel_groupwise_message_cache_insert_stream (dest, dest_uid, part, stream, ex);
			camel_object_unref (CAMEL_OBJECT (stream));
		}
	}
}
