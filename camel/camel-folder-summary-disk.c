...
/*
  ...
  - BTREE database
  - keys are full_name[:viewid], there is one entry for
    each view of every folder, including the root view.
  - each record are as generated by encode_view().
...
struct _camel_disk_env {
	...
	/* EVERY call to libdb (on this env) must be locked */
...
#define LOCK_ENV(e) g_mutex_lock((e)->lock)
#define UNLOCK_ENV(e) g_mutex_unlock((e)->lock)
#endif
...
get_env(CamelService *s, CamelException *ex)
{
	char *base;
	...
	/* FIXME: cleanup failures properly */

	printf("Creating database environment for store at %s\n", env->path);

	if ((err = db_env_create(&env->env, 0)) != 0) {
		camel_exception_setv(ex, 1, "env create failed: %s", db_strerror(err));
		return NULL;
	}

	if ((err = env->env->open(env->env, env->path, DB_INIT_LOG|DB_INIT_MPOOL|DB_INIT_TXN|DB_CREATE|DB_RECOVER /* |DB_PRIVATE */, 0666)) != 0) {
		camel_exception_setv(ex, 1, "env open failed: %s", db_strerror(err));
		return NULL;
	}

	/* Open the folder list 'table' */
	db_create(&env->folders, env->env, 0);
	if ((err = env->folders->open(env->folders, NULL, "folders", ".#folders", DB_BTREE, DB_CREATE, 0666)) != 0) {
		camel_exception_setv(ex, 1, "folder list database create failed: %s", db_strerror(err));
		return NULL;
	}

	/* We could load the folders now/etc? */

	return env;
}

static void
cds_view_save(CamelFolderSummaryDisk *cds, CamelFolderViewDisk *view, DB_TXN *txn, CamelException *ex)
{
	...
	CDS_CLASS(cds)->encode_view(cds, (CamelFolderView *)view, cre);

	if (view->view.vid && view->view.vid[0]) {
		key.data = g_alloca(strlen(((CamelFolderSummary *)cds)->folder->full_name) + strlen(view->view.vid)+3);
		sprintf(key.data, "%s:%s", ((CamelFolderSummary *)cds)->folder->full_name, view->view.vid);
	} else
		key.data = ((CamelFolderSummary *)cds)->folder->full_name;
	key.size = strlen(key.data);
	data.data = cre->out->data;
	data.size = cre->out->len;

	printf("saving view '%s'\n", key.data);

	LOCK_ENV(view->env);
	res = view->env->folders->put(view->env->folders, NULL, &key, &data, 0);
	UNLOCK_ENV(view->env);
...
cds_view_initdb(CamelFolderSummary *s, CamelFolderView *fview, DB_TXN *txn, CamelException *ex)
{
	...
	if (view->view.vid == NULL) {
		...
		name = g_alloca(strlen(s->folder->full_name)+strlen(view->view.vid)+2);
		sprintf(name, "%s:%s", s->folder->full_name, view->view.vid);
	}
...
cds_load_views(CamelFolderSummaryDisk *cds, DB_TXN *txn, CamelException *ex)
{
	...
	/* Load all the views for this folder.

	We just jump to the folder key, and load the root view, if it exists,
	then iterate through all views which are present as following
	entries, loading each in turn.  If there is no root view entry,
	just create one and finish, we assume no views exist.

	This looks a lot more complex than it actually is !*/

	printf("Initialising views for folder '%s'\n", cds->summary.folder->full_name);

	env = get_env((CamelService *)cds->summary.folder->parent_store, ex);
	if (env == NULL)
		return;

	LOCK_ENV(env);
	res = env->folders->cursor(env->folders, NULL, &cursor, 0);
	if (res == -1) {
		camel_exception_setv(ex, 1, "getting folders cursor: %s", db_strerror(res));
		UNLOCK_ENV(env);
		// fixme: unref env?
		return;
	}

	key.data = CFS(cds)->folder->full_name;
	len = strlen(key.data);
	key.size = len;
	data.flags = DB_DBT_REALLOC;
	res = cursor->c_get(cursor, &key, &data, DB_SET);
	UNLOCK_ENV(env);

	if (res == DB_NOTFOUND) {
		printf(" no root view, adding that\n");
		camel_folder_summary_view_add(CFS(cds), camel_folder_summary_view_new(CFS(cds), NULL), ex);
		return;
	} else if (res != 0) {
		// unref env?
		camel_exception_setv(ex, 1, "getting folder entry: %s", db_strerror(res));
		return;
	}

	printf(" loading root view\n");
	memset(&key, 0, sizeof(key));
	key.flags = DB_DBT_REALLOC;
	view = camel_folder_summary_view_new(CFS(cds), NULL);
...
static void
cds_save_views(CamelFolderSummaryDisk *cds, DB_TXN *txn, CamelException *ex)
{
	CamelFolderViewDisk *root = (CamelFolderViewDisk *)((CamelFolderSummary *)cds)->root_view;
	CamelFolderView *view;

	if (root->view.touched)
		cds_view_save(cds, root, txn, ex);

	for (view = (CamelFolderView *)CFS(cds)->views.head;
	     !camel_exception_is_set(ex) && view->next;
	     view = view->next) {
		g_assert(view->vid && view->vid[0]);
		if (view->touched)
			cds_view_save(cds, (CamelFolderViewDisk *)view, txn, ex);
	}
}
...
static int cds_decode_view(CamelFolderSummaryDisk *cds, CamelFolderView *view, CamelRecordDecoder *crd)
{
	int tag, ver;
