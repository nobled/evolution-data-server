...
cds_view_save(CamelFolderSummaryDisk *cds, CamelFolderViewDisk *view, DB_TXN *txn, CamelException *ex)
{
	...
	if (view->view.vid) {
		key.data = g_alloca(strlen(((CamelFolderSummary *)cds)->folder->full_name) + (view->view.vid?strlen(view->view.vid):0) + 3);
		sprintf(key.data, "%s\01%s", ((CamelFolderSummary *)cds)->folder->full_name, view->view.vid?view->view.vid:"");
	} else
...
cds_view_usedb(CamelFolderSummary *s, CamelFolderView *fview, DB_TXN *txn, CamelException *ex)
{
	...
	*/
	...
	if (view->view.vid) {
		...
		sprintf(name, "%s\01%s", s->folder->full_name, view->view.vid?view->view.vid:"");
	} else
...
cds_load_views(CamelFolderSummaryDisk *cds, DB_TXN *txn, CamelException *ex)
{
	...
	if (res == DB_NOTFOUND) {
		v(printf(" no root view, adding that\n"));
	...
	if (res == 0)
		v(printf("last record matched was '%.*s'\n", key.size, (char *)key.data));
...
cds_view_free(CamelFolderSummary *s, CamelFolderView *view)
{
	...
	if (view->deleted) {
		...
		sprintf(name, "%s\01%s", s->folder->full_name, view->vid);
