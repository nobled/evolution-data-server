			   ...
			   void *data)
{
	...
	new = g_new0 (StorageCallbackData, 1);
		...
		const char *path)
{
	...
	if (strcmp (path, "/") == 0)
		full_path = g_strconcat ("/", storage_name, NULL);
	else if (! g_path_is_absolute (path))
		full_path = g_strconcat ("/", storage_name, "/", path, NULL);
	else
		full_path = g_strconcat ("/", storage_name, path, NULL);
					       ...
					       GList *path_list)
{
	...
	for (p = path_list; p != NULL; p = p->next) {
		...
		path_with_storage = g_strconcat ("/", e_storage_get_name (storage), path, NULL);
		...
		// SURF : e_free_string_list (sub_path_list);
		for (f = sub_path_list; f != NULL ; f = f->next)
			g_free (f->data);

		g_list_free (sub_path_list);
	}
}
					      ...
					      EStorage *storage)
{
	...
	path_list = e_storage_get_subfolder_paths (storage, "/");

	signal_new_folder_for_all_folders_under_paths (storage_set, storage, path_list);

	// SURF : e_free_string_list (path_list);
	for (f = path_list; f != NULL ; f = f->next)
		g_free (f->data);
...
init (EStorageSet *storage_set)
{
	...
	priv = g_new0 (EStorageSetPrivate, 1);
				    ...
				    void *data)
{
	...
	e_storage_async_create_folder (storage, subpath, type,
